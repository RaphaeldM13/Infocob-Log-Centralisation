...
# 3. Présentation des composants
  ...
  MinIO est un serveur de stockage objet open source, compatible avec l'API Amazon S3.
  Dans ce projet, il constitue la couche de persistance de Loki : les logs ingérés
  sont stockés dans deux buckets MinIO (`loki-data` et `loki-ruler`).

  Il est déployé comme conteneur Docker au sein de la même stack que Loki,
  ce qui évite toute dépendance à un service de stockage cloud externe.
